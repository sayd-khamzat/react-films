...
export function SearchPanel() {
    ...
    return (
        <div className='relative flex w-[360px] h-[42px] ml-96 rounded bg-white shadow-md text-slate-800'>
            ...
            <input type='text'
                   className='outline-0 w-full ml-3 text-base'
                   placeholder='Поиск'
                   value={search}
                   onChange={e => setSearch(e.target.value)}/>

            {dropdown &&
                <ul className='list-none absolute rounded shadow-md bg-slate-300 top-[42px] max-h-[200px]
                    ...
                    {moviesList?.length === 0
                        ...
                        : moviesList?.map(movie => (
                            ...
                            <li key={movie.id}
                                ...
                                className='py-2 px-4 hover:bg-indigo-600 hover:text-white hover:shadow-md rounded transition-colors cursor-pointer'>
